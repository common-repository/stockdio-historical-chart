(function () {
	...
	s_s_processPopularSingleCategory= function(items, modal){
		...
		if (true){
			...
			if (items[0].isGroup){
				...
				exchName = "USA Equities and ETFs";
	...
	s_s_search_exchange_info= function(exchName, catName){
		...
		if (!exchName){
			if ($(".s_b_exch_code").length>0)
				exchName = $(".s_b_exch_code").text();
		}

		if (!exchName){
			return { country:"us", exchName:"USA Equities and ETFs", exch:"USA", countryName:"United States"}
		}
	...
	stockdio_get_popular_array = function(){
		$.ajax({
			url: `https://api.stockdio.com/freedata/financial/info/v1/getpopulartickerssv`,
	...
	stockdio_processSymbolsString = function(hidePopular,scrollToBottom, validate){
		...
		$.ajax({
			url: `https://api.stockdio.com/freedata/financial/info/v1/validateSymbolsSV/?exchange=${exchange}&symbols=${symbols}`,
			success: function (data, textStatus, xhr) {				
				$(".stockdio_search_loading").hide();
	...
	stockdio_search_onclose_and_save = function(){
		...
		if (!mValue){
			...
			if (!exchName){
				if ($("#select-exchange").length > 0){
					$("#select-exchange").val(exchange);
					exchName = $("#select-exchange option:selected").text();
				}			
			}
			if (!exchName)
